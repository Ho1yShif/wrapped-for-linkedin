...
export function parsePercentage(value: any): number {
  if (typeof value === 'number') {
    // If it's already a number between 0-100, assume it's a percentage value
    if (value >= 0 && value <= 100) {
      return value;
    }
    // If it's less than 1, treat as already a decimal percentage
    if (value < 1) {
      return value;
    }
    return value;
  ...
  // Handle \"< 1%\" or similar by returning a small value (0.5%)
